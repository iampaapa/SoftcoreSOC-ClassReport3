...
(
   ...
   output logic [N_LED-1:4] gpo_led_o,         // Output for the GPO core
   output logic [3:0] blinking_led_o,    // Output for the new Blinking LED core
   // uart
   input logic rx,
   output logic tx          
);
   ...
   // slot 2: gpo 
   chu_gpo #(.W(12)) gpo_slot2 // I set width to 12
   (.clk(clk),
    .reset(reset),
    .cs(cs_array[`S2_LED]),
   ...
   (.clk(clk),
    ...
    blinking_led_core #(.N_LEDS(4)) blinking_led_slot4
    (.clk(clk),
    .rst(reset),
    .cs(cs_array[`S4_BLINKING_LED]),
    .write(mem_wr_array[`S4_BLINKING_LED]),
    .addr(reg_addr_array[`S4_BLINKING_LED]),
    .wr_data(wr_data_array[`S4_BLINKING_LED]),
    .rd_data(rd_data_array[`S4_BLINKING_LED]),
    .led(blinking_led_o)
    );
   ...
   generate
      genvar i;
      // Slot 4 is now used, so the loop for unused slots starts at 5.
